...
public class OrbitCamControl {
    ...
    public void setupMouseTriggers(final LogicalLayer layer, final boolean dragOnly) {
        ...
        final TriggerAction mouseAction = new TriggerAction() {
            ...
            // Test for last mouse_up action            

            public void perform(final Canvas source, final TwoInputStates inputStates, final double tpf) {
                final MouseState mouse = inputStates.getCurrent().getMouseState();
                if ((mouse.getDx() != 0 || mouse.getDy() != 0) && !inputStates.getPrevious().getMouseState().getButtonState(MouseButton.LEFT).equals(ButtonState.UP)) {
                    if (!firstPing) {
