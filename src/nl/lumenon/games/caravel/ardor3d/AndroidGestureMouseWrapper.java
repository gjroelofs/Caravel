 ...
 */
public class AndroidGestureMouseWrapper implements MouseWrapper {
    ...
    public AndroidGestureMouseWrapper(final View view) {
        _view = view;
    }
    ...
    private class AndroidMouseIterator extends AbstractIterator<MouseState> implements PeekingIterator<MouseState> {
        ...
        protected MouseState computeNext() {
            synchronized (AndroidGestureMouseWrapper.this) {
    ...
    private void addNewState(final MotionEvent event, final EnumMap<MouseButton, ButtonState> enumMap) {
    	MouseState newState=null;
    	if(event.getPointerCount()>1){
    		// We have a gesture, assume pinch-to-zoom
    		 newState = new MouseState(Math.round(event.getX()), getArdor3DY(event), 0,
 	                0, (int)getWheel(event), enumMap, null);
    	} else {
    		 newState = new MouseState(Math.round(event.getX()), getArdor3DY(event), getDX(event),
 	                getDY(event), 0, enumMap, null);
    	}
    ...
    private float getWheel(final MotionEvent event){
    	...
    	float wheel =0;
    	if(_oldDistance!=0){
    		wheel = _oldDistance-spacing(event);
    	}
    	_oldDistance=spacing(event);
